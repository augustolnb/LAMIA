...
async def update_interaction_history(session_service, app_name, user_id, session_id, entry):
    ...
    try:
        ...
        # Delete the existing session first, then create a new one with updated state
        # This is necessary because InMemorySessionService.create_session() 
        # doesn't allow creating a session with an ID that already exists
